...
| `updated` | 谱面上次更新时间 |
| `ranked` | 谱面上架或达标时间 |
| `tag` | 特定的[用户标签](/wiki/Beatmap/Beatmap_tags#用户标签-(user-tags)) |
...
| `length` | [游玩时间](/wiki/Beatmap/Play_time)（单位为秒） |
| `key`, `keys` | 按键数量（osu!mania 以及该模式的转谱特有） |
| `ln`, `lns` | 长按音符占比（osu!mania 以及该模式的转谱特有） |
...
| `ranked` | 谱面上架或达标时间 |
| `tag` | 特定的[用户标签](/wiki/Beatmap/Beatmap_tags#用户标签-(user-tags))（仅游戏内置的谱面搜索列表支持） |
...
（Lazer、网站）搜索包含谱师标签：`J-pop` 以及用户标签：`meta/custom skin` 的谱面：
